...
#define __I2C_H
#include "stdint.h"
#define I2CHLongTime 0    //高速I2C
#define I2CHShortTime 0
#define I2CMLongTime 2    //中速I2C
#define I2CMShortTime 0
#define I2CLLongTime 10    //低速I2C
#define I2CLShortTime 0
// #define I2C_USEPOINTER
#define I2C_EXCEPTION    //读取从机应答信号后, 是否进行处理
...
#define I2CBUS_ERROR() ((I2C_SoftHandleTypeDef *)modular->bushandle)->error()
...
#define I2CBUS_SDA_Set(dir) DEVCMNI_SDA_Set(dir)
#define I2CBUS_SCL_Out(pot) DEVCMNI_SCL_Out(pot)
#define I2CBUS_SDA_Out(pot) DEVCMNI_SDA_Out(pot)
#define I2CBUS_SDA_In() DEVCMNI_SDA_In()
#define I2CBUS_ERROR() DEVCMNI_Error()
...
void DEVCMNI_SCL_Out(Potential_TypeDef pot);
void DEVCMNI_SDA_Out(Potential_TypeDef pot);
...
//在STM32F401CDU6上测得I2C写入OLED屏幕(8*3byte+8*128byte)耗时: (6.21*((1+1+3)+(1+1+128))+10.15+10.15)*8 = 6869us
static uint8_t shorttime = 0, longtime = 0;
static inline void I2C_Settime(I2C_ModuleHandleTypeDef *modular, uint32_t *timeout) {
    if(modular->speed == I2CHIGH) {
        longtime = I2CHLongTime;
        shorttime = I2CHShortTime;
    } else if(modular->speed == I2CMEDIUM) {
        longtime = I2CMLongTime;
        shorttime = I2CMShortTime;
    } else if(modular->speed == I2CLOW) {
        longtime = I2CLLongTime;
        shorttime = I2CLShortTime;
    }
    *timeout = *timeout > longtime ? *timeout : longtime;
}
////////////////////////////////////////////////////////////////////////////
...
static inline int8_t I2C_Write(I2C_ModuleHandleTypeDef *modular, uint8_t *pdata, uint16_t size, uint32_t timeout) {
    for(uint16_t j = 0; j < size; j++) {
        for(uint8_t i = 0x80; i; i >>= 1) {
            I2C_WriteBit(modular, pdata[j] & i ? 1 : 0);
        }
        I2CBUS_SDA_Out(HIGH);    //释放数据总线
        I2CBUS_Delayus(shorttime);
        if(I2C_WriteWait(modular, timeout) == 1) {
...
static inline void I2C_Read(I2C_ModuleHandleTypeDef *modular, uint8_t *pdata, uint16_t size, uint32_t timeout) {
    for(uint16_t j = 0; j < size; j++) {
        I2CBUS_SDA_Set(IN);    //数据线设置为读取模式
        for(uint8_t i = 0x80; i; i >>= 1) {
            pdata[j] |= I2C_ReadBit(modular) ? i : 0x00;
        }
        I2CBUS_SDA_Set(OUT);    //数据线设置为写入模式
        if(j + 1 < size) {
