...
uint64_t time_us = 0, time_ms = 0;
uint64_t TIMER_getRunTimeus(void) {
    time_us = time_ms*1000+(TIM4->CNT)%1000;
    return time_us;
}
...
//获取当前时间, 比较历史时间
#define TimerError 2    //为了修正函数运行带来的误差, 在使用不同型号芯片前需调试计算得到数值, 并且在10us以内的延时有程度逐渐减小的误差
int8_t TIMER_uscmptor(uint16_t us, uint64_t *compare, int8_t *state) {
    if(*state == 0) {
        *compare = time_us+TIM4->CNT;
        (*state)++;
    }
    if(*state==1 && time_us+TIM4->CNT-*compare+TimerError>=us) {
        (*state)--;
        return 1;
    }
    return 0;
}
int8_t TIMER_mscmptor(uint16_t ms, uint64_t *compare, int8_t *state) {
    if(*state == 0) {
        *compare = time_us+TIM4->CNT;
        (*state)++;
    }
    if(*state==1 && time_us+TIM4->CNT-*compare>=ms*1000) {
        (*state)--;
        return 1;
    }
    return 0;
}
int8_t TIMER_scmptor(uint16_t s, uint64_t *compare, int8_t *state) {
    if(*state == 0) {
        *compare = time_us+TIM4->CNT;
        (*state)++;
    }
    if(*state==1 && time_us+TIM4->CNT-*compare>=s*1000000) {
        (*state)--;
        return 1;
    }
    return 0;
