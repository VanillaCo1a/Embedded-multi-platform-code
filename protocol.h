...
__STATIC_INLINE bool DEVI2C_ClockStetch(void) {    //时钟拉伸判断函数
...
__STATIC_INLINE bool DEVI2C_BusArbitration(void) {    //总线仲裁判断函数
    if(i2cmodular.errhand == DEVI2C_LEVER0) {         //直接写下一位
        return 0;
...
__STATIC_INLINE void DEVI2C_WriteBit(bool bit) {    // 写1位数据
...
__STATIC_INLINE void DEVI2C_WriteBit_(bool bit) {
    DEVI2C_SDA_Out(bit);
...
__STATIC_INLINE bool DEVI2C_ReadBit(void) {    // 读1位数据
...
__STATIC_INLINE bool DEVI2C_ReadBit_(void) {
    bool bit = 0;
    // DEVI2C_SDA_Out(HIGH);
    // DEVI2C_SDA_Set(IN);
    DEVI2C_Delayus(DEVI2C_scl_lowtime);
...
__STATIC_INLINE void DEVSPI_WriteBit(SPI_ModuleHandleTypeDef *modular, bool bit) {
    DEVSPI_SDI_Out(bit);
    DEVSPI_delayus(DEVSPI_SCLK_LOW_TIME);
    DEVSPI_SCK_Out(HIGH);
    DEVSPI_delayus(DEVSPI_SCLK_HIGH_TIME);
...
__STATIC_INLINE bool DEVSPI_ReadBit(SPI_ModuleHandleTypeDef *modular) {
    bool bit = 0;
    DEVSPI_delayus(DEVSPI_SCLK_LOW_TIME);
    DEVSPI_SCK_Out(HIGH);
    DEVSPI_delayus(DEVSPI_SCLK_HIGH_TIME);
...
__STATIC_INLINE bool DEVSPI_TransmitBit(SPI_ModuleHandleTypeDef *modular, bool bit) {
    DEVSPI_SDI_Out(bit);
    DEVSPI_delayus(DEVSPI_SCLK_LOW_TIME);
    DEVSPI_SCK_Out(HIGH);
    DEVSPI_delayus(DEVSPI_SCLK_HIGH_TIME);
...
#endif    // DEVOWRE_USEPOINTER
__STATIC_INLINE int8_t DEVOWRE_Init(ONEWIRE_ModuleHandleTypeDef *modular) {
...
__STATIC_INLINE uint8_t DEVOWRE_Reset(ONEWIRE_ModuleHandleTypeDef *modular) {
    uint8_t result = 0;
...
__STATIC_INLINE void DEVOWRE_WriteBit(ONEWIRE_ModuleHandleTypeDef *modular, bool bit) {
    if(bit) {
        //todo: 关中断
...
__STATIC_INLINE uint8_t DEVOWRE_ReadBit(ONEWIRE_ModuleHandleTypeDef *modular) {
    bool bit = 0;
    //todo: 关中断
...
__STATIC_INLINE void DEVOWRE_WriteByte(ONEWIRE_ModuleHandleTypeDef *modular, uint8_t byte) {
    for(uint8_t i = 0x01; i; i <<= 1) {
        DEVOWRE_WriteBit(modular, byte & i ? 1 : 0);
    }
}
__STATIC_INLINE uint8_t DEVOWRE_ReadByte(ONEWIRE_ModuleHandleTypeDef *modular) {
    uint8_t byte = 0;
    for(uint8_t i = 0x01; i; i <<= 1) {
...
__STATIC_INLINE void DEVOWRE_Write(ONEWIRE_ModuleHandleTypeDef *modular, uint8_t *pdata, uint16_t size) {
    for(uint16_t j = 0; j < size; j++) {
        for(uint8_t i = 0x01; i; i <<= 1) {
...
__STATIC_INLINE void DEVOWRE_Read(ONEWIRE_ModuleHandleTypeDef *modular, uint8_t *pdata, uint16_t size) {
    for(uint16_t j = 0; j < size; j++) {
