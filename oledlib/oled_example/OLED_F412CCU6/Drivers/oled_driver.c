...
void OLED_gpioInit(void) {
    #if (MCU_COMPILER == MCU_STM32FWLIB)
    if(OLED[_outputnum].oledprop==OLED_I2C4PIN || OLED[_outputnum].oledprop==OLED_I2C7PIN) {
        ...
        if(OLED[_outputnum].oledprop == OLED_I2C7PIN) {
            ...
            if(OLED[_outputnum].oledio->GPIO_dc != NULL) {
                ...
                GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
                DEFINE_GPIO_SET(dc);
                if(((I2C_AnalogTypedef *)OLED[_outputnum].analogcomi)->ADDRESS == OLED_I2CADDR1) {
    ...
    }else if(OLED[_outputnum].oledprop == OLED_SPI7PIN) {
...
void OLED_wireSet(void) {
    ...
    if(OLED[_outputnum].oledcomi == OLED_ANALOG) {
        if(OLED[_outputnum].oledprop==OLED_I2C4PIN || OLED[_outputnum].oledprop==OLED_I2C7PIN) {
            ...
            OLED[_outputnum].analogcomi = (I2C_AnalogTypedef *)&OLEDI2C[j];
            j++;
        }else if(OLED[_outputnum].oledprop == OLED_SPI7PIN) {
            ...
            OLED[_outputnum].analogcomi = (SPI_AnalogTypedef *)&OLEDSPI[k];
            k++;
        }
    }
}
...
void OLED_writeByte(uint8_t data, uint8_t address) {
    if(OLED[_outputnum].oledprop==OLED_I2C4PIN || OLED[_outputnum].oledprop==OLED_I2C7PIN) {
        if(OLED[_outputnum].oledprop==OLED_I2C7PIN && OLED[_outputnum].oledio->GPIO_cs != NULL) {
            GPIO_OUT_RESET(cs);
        }
        if(OLED[_outputnum].oledcomi == OLED_ANALOG) {
            MODULAR_I2CWrite(*((I2C_AnalogTypedef *)OLED[_outputnum].analogcomi), address, &data,1, 0,I2CHIGH);
        }else if(OLED[_outputnum].oledcomi == OLED_HARDWARE) {
          #if (MCU_I2CCOM==MCU_HARDWARE || MCU_I2CCOM==MCU_BOTH)
            #if (MCU_COMPILER == MCU_STM32FWLIB)
            //固件库的硬件I2C驱动函数,待补充
            #elif (MCU_COMPILER == MCU_STM32HAL)
            HAL_I2C_Mem_Write(&hi2c1, (OLED_I2CADDR1<<1)|0X00, address,I2C_MEMADD_SIZE_8BIT, &data,1, 0x100);
            //HAL_FMPI2C_Mem_Write(&hfmpi2c1, (OLED_I2CADDR1<<1)|0X00, address,FMPI2C_MEMADD_SIZE_8BIT, &data,1, 0x100);
            #endif
          #endif
        }
        if(OLED[_outputnum].oledprop==OLED_I2C7PIN && OLED[_outputnum].oledio->GPIO_cs != NULL) {
        ...
        if(OLED[_outputnum].oledcomi == OLED_ANALOG) {
            MODULAR_SPIWriteByte(*((SPI_AnalogTypedef *)OLED[_outputnum].analogcomi), data);
        }else if(OLED[_outputnum].oledcomi == OLED_HARDWARE) {
          #if (MCU_SPICOM==MCU_HARDWARE || MCU_SPICOM==MCU_BOTH)
            if(OLED[_outputnum].oledio->GPIO_cs != NULL) {
                GPIO_OUT_RESET(cs);
            }
            #if (MCU_COMPILER == MCU_STM32FWLIB)
...
void OLED_writeConti(uint8_t *pdata, uint16_t size, uint8_t address) {
    if(OLED[_outputnum].oledprop==OLED_I2C4PIN || OLED[_outputnum].oledprop==OLED_I2C7PIN) {
        ...
        if(OLED[_outputnum].oledcomi == OLED_ANALOG) {
            MODULAR_I2CWrite(*((I2C_AnalogTypedef *)OLED[_outputnum].analogcomi), address, pdata,size, 0,I2CHIGH);
        }else if(OLED[_outputnum].oledcomi == OLED_HARDWARE) {
          #if (MCU_I2CCOM==MCU_HARDWARE || MCU_I2CCOM==MCU_BOTH)
            #if (MCU_COMPILER == MCU_STM32FWLIB)
            //固件库的硬件I2C驱动函数,待补充
            #elif (MCU_COMPILER == MCU_STM32HAL)
            HAL_I2C_Mem_Write(&hi2c1, (OLED_I2CADDR1<<1)|0X00, address,I2C_MEMADD_SIZE_8BIT, pdata,size, 0x100);
            //HAL_FMPI2C_Mem_Write(&hfmpi2c1, (OLED_I2CADDR1<<1)|0X00, address,FMPI2C_MEMADD_SIZE_8BIT, pdata,size, 0x100);
            #endif
          #endif
        }
        if(OLED[_outputnum].oledprop==OLED_I2C7PIN && OLED[_outputnum].oledio->GPIO_cs != NULL) {
        ...
        if(OLED[_outputnum].oledcomi == OLED_ANALOG) {
            MODULAR_SPIWrite(*((SPI_AnalogTypedef *)OLED[_outputnum].analogcomi), pdata, size);
        }else if(OLED[_outputnum].oledcomi == OLED_HARDWARE) {
          #if (MCU_SPICOM==MCU_HARDWARE || MCU_SPICOM==MCU_BOTH)
            if(OLED[_outputnum].oledio->GPIO_cs != NULL) {
                GPIO_OUT_RESET(cs);
...
void OLED_Init(int8_t num) {   //初始化对象, 定义并初始化引脚, 复位,配置oled并清屏
    static int8_t once = 1;
    //在第一次调用初始化函数时, 就进行所有OLED的引脚配置, 并复位定义RST脚的OLED
    if(once == 1) {
        once = 0;
        OLED_ioDef();
